...
class Login:
    ...
    def prosesLogin(self, event=None):
        ...
        # logika pemrograman
        if nmUser=='':
            messagebox.showwarning('Pesan Salah', 'Nama User tidak boleh kosong!', parent=self.induk)
            self.entUser.focus_set()
        elif passUser=='':
            messagebox.showwarning('Pesan Salah', 'Kata Kunci tidak boleh kosong!', parent=self.induk)
            self.entPass.focus_set()
        elif (nmUser==datUser) and (passUser==datPassword):
            messagebox.showinfo("Login", "Selamat Datang, Admin!", parent=self.induk)
            self.Tutup()
        else:
            messagebox.showwarning('Pesan Salah', 'Nama Pengguna atau Kata Kunci SALAH!!', parent=self.induk)
            self.Hapus()
